...
# --- Helper functions for GNN graph creation and external use (e.g., in test.py) ---
# These are no longer called during training data loading but are kept for compatibility.
...
def create_dummy_graph():
    node_features = torch.zeros(1, 39, dtype=torch.float)
    edge_index = torch.empty(2, 0, dtype=torch.long)
    return Data(x=node_features, edge_index=edge_index)

def smiles_to_graph_data(smiles: str) -> Data:
    if not smiles: return create_dummy_graph()
    mol = Chem.MolFromSmiles(smiles)
    if mol is None or mol.GetNumAtoms() == 0: return create_dummy_graph()
    ...
    for bond in mol.GetBonds():
        ...
        edge_indices.extend([(i, j), (j, i)]); edge_attr.extend([bond_feats, bond_feats])
    edge_index = torch.tensor(edge_indices, dtype=torch.long).t().contiguous()
    edge_attr = torch.tensor(edge_attr, dtype=torch.float)
    return Data(x=x, edge_index=edge_index, edge_attr=edge_attr)

# --- UPDATED: Dataset now loads all pre-processed data for maximum efficiency ---
class SmilesGraphDataset(torch.utils.data.Dataset):
    def __init__(self, hdf5_path):
        # Keep the file open to leverage h5py's lazy loading
        self.h5_file = h5py.File(hdf5_path, 'r')
        self.embeddings = self.h5_file['embeddings']
        self.smiles = self.h5_file['smiles']
        
        # Load handles to the new pre-computed datasets
        self.peak_bins = self.h5_file['peak_bins']
        self.formula_strings = self.h5_file['formula_strings']
        self.scaffold_smiles = self.h5_file['scaffold_smiles']
        
        self.length = len(self.embeddings)
        # Verify that all datasets have the same length
        assert self.length == len(self.smiles) == len(self.peak_bins) == len(self.formula_strings) == len(self.scaffold_smiles), \
    ...
    def __getitem__(self, idx):
        # All heavy computation is gone. This is now just fast data reading.
        scaffold_smi = self.scaffold_smiles[idx].decode('utf-8')
        scaffold_graph = smiles_to_graph_data(scaffold_smi)
        
        return {
            "embedding": self.embeddings[idx], 
            "formula": self.formula_strings[idx].decode('utf-8'), 
            "scaffold_graph": scaffold_graph, 
            "smiles": self.smiles[idx].decode('utf-8'),
            "peak_bins": torch.tensor(self.peak_bins[idx], dtype=torch.long)
        }

# --- UPDATED: Collate function is now simpler ---
def get_graph_data_loader(hdf5_path, smiles_tokenizer, formula_tokenizer, split, batch_size=32, num_workers=0):
    ...
    def collate_graphs(data_list):
        ...
        peak_bins = [item['peak_bins'] for item in data_list]
        ...
        peaks_stacked = torch.stack(peak_bins, dim=0)
        
        return {"embedding": embeddings_tensor, "formula": formulas_padded, "scaffold_graph_list": scaffold_graphs, "smiles": smiles_padded, "peak_bins": peaks_stacked}
...
if __name__ == '__main__':
    ...
    if test_loader:
        ...
        if first_batch:
            ...
            print(f"  Peaks stacked Shape:     {first_batch['peak_bins'].shape}")
            ...
            if scaffold_list:
                ...
                first_peaks = first_batch['peak_bins'][0].tolist()
                print(f"  First spectra in batch peaks: {first_peaks}")
        else:
